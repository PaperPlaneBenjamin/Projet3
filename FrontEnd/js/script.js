...
function createButton(id, name, div) {
    const button = document.createElement("div");
    button.dataset.id = id;
    button.className = "item";
    button.addEventListener('click', function (event) {
        activeButton(button);        
        filterPictures(parseInt(event.currentTarget.dataset.id));
    });
...
function addButton(categories) {
    ...
    const buttonTous = createButton(0, "Tous", div);
    activeButton(buttonTous);
    categories.forEach((categorie) => {
        createButton(categorie.id, categorie.name, div);
    });
}
...
function filterPictures(category_id) {    
    const picturesFiltered = category_id  === 0 ? window.pictures : window.pictures.filter(picture => picture.categoryId === category_id);
    removePictureHTML();
...
async function principal() {
    ...
    // Récupère les travaux de l'API // 
    const response = await fetch('http://localhost:5678/api/works');
    window.pictures = await response.json();
    addPicture(window.pictures);
...
// Variable globale de type booléenne, servant de condition au fetch POST, pour l'ajout de travaux // 
let fetchPost = false;

// Appel et exécution de la fonction principale // 
principal();